...
func sarifRuleText(source string) string {
	...
	case "WINOS":
		return "Windows build banner (informational context)"
	case "WINPRIV":
		return "Windows token privileges that are escalation primitives"
	case "WINREG":
		return "Windows registry misconfigurations (AlwaysInstallElevated, AutoLogon, UAC)"
	case "WINSVC":
		return "Windows service binary-planting surfaces"
	case "WINAUTO":
		return "Windows autorun entries the current user can rewrite"
	case "WINTASK":
		return "Windows scheduled tasks with user-writable commands"
	case "WINCRED":
		return "Windows credential artifacts (unattend, GPP, history, keys)"
	case "WINPATH":
		return "writable directories in the Windows PATH"
	default:
