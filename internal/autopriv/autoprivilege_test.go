...
func TestVectorSelectionSymmetry(t *testing.T) {
	sources := []Finding{
		...
		// Windows sources (v1.9): the same full-finding-set contract —
		// every win-* vector must enumerate from its own source. The
		// Targets/Descriptions mirror what windows_scan.go emits so the
		// dispatch keys (privilege names, finding text) match reality.
		{Source: "WINPRIV", Target: "SeImpersonatePrivilege", Exploitable: true},
		{Source: "WINREG", Target: `HKLM+HKCU\...\Windows\Installer\AlwaysInstallElevated`, Exploitable: true},
		{Source: "WINSVC", Target: "VulnSvc", Description: "Unquoted service path with spaces and writable ancestor C:\\w — plant an exe up the chain", Exploitable: true},
		{Source: "WINAUTO", Target: "Updater", Exploitable: true},
		{Source: "WINTASK", Target: `\Path\Task`, Exploitable: true},
		{Source: "WINCRED", Target: `C:\Windows\Panther\unattend.xml`, Description: "Deployment answer file with a password field — extract it", Exploitable: true},
		{Source: "WINPATH", Target: `C:\writable\path`, Exploitable: true},
	}
