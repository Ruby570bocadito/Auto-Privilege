...
| **Script-friendly** | `--quiet` + exit codes (`0` root, `1` no root, `2` error, `3` policy gate), `--no-color` auto when piped, `--color` to force the terminal look through a pipe (captures, demos), `NO_COLOR` respected, `--parallel` cuts scan wall-clock with byte-identical results |
