...
¿No sabes qué acepta `--vector`? `--list-vectors` imprime el catálogo — cada nombre de vector con una descripción de una línea de qué inspecciona realmente, leída de la misma tabla que usa el binario, así la documentación nunca puede divergir del código. Añade `--json` y el mismo catálogo llega como documento máquina-legible con sobre `tool`/`version`/`mode`. El modo hermano `--list-sources` hace lo mismo con el vocabulario de fuentes de hallazgos (lo que aceptan `--ignore`/`--explain`): 17 fuentes, cada una descrita por la frase exacta que usa su playbook de endurecimiento — y su voz `--json` alimenta dashboards sin rascar texto de ayuda:

<p align="center"><img src="docs/images/screenshot-vectors.png" alt="--list-vectors: el catálogo de 18 vectores" width="640"></p>

<p align="center"><img src="docs/images/screenshot-list-sources.png" alt="--list-sources: el vocabulario de 17 fuentes, voces texto y --json" width="640"></p>
